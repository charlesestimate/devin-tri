...
| 12 | **The gate and hard-block registers** | Decide which register is authoritative — the seed (your specification: amount bands, named primary and alternate approvers, working-day windows, your six hard blocks) or the live rows the code was built against (severity labels, no approver roles, a different six). Then reconcile every hard-coded gate reference in the code and backfill the thirty rows and six blocks | **Needs your decision, not Hercules'.** 25 of 30 gate labels and 6 of 6 hard blocks differ between the two. This is why no approver role has ever appeared on screen |
