...
svg.append(f'<rect x="{lx}" y="{gy+22}" width="60" height="15" rx="7.5" fill="{GREEN}"/><text x="{lx+4}" y="{gy+33}" font-size="8" font-weight="bold" fill="#fff">G6 Director·COO</text><text x="{lx+66}" y="{gy+33}" font-size="9.4" fill="{INK}">An approval gate: number · primary approver (Accountable) · alternate (Consulted) · working-day window where set. Thirty gates; 13–17 reserved.</text>')
